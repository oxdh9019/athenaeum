...
function MapCanvas({ agents, locations, onAgentClick }: MapCanvasProps) {
  ...
  return (
    <div style={{
      ...
      <h3 style={{ fontSize: '14px', color: '#00d4ff', margin: '0 0 12px 0' }}>🗺️ 世界地图</h3>
      ...
      >
        ...
        {locations.map((loc, idx) => {
          const pos = getLocationPosition(idx, locations.length)
          ...
          return (
            <g key={loc.id} transform={`translate(${pos.x}, ${pos.y})`}>
              ...
              {/* 地点名称 */}
              <text x="80" y="24" textAnchor="middle" fill="#e0e0e0" fontSize="14" fontWeight="bold">
              ...
              {agentsHere.map((agent, aIdx) => (
                ...
                >
                  <circle
                    ...
                    cy="12"
                    r="12"
                    fill={AGENT_COLORS[agents.indexOf(agent) % AGENT_COLORS.length]}
                  />
                  <text
                    x="12"
                    y="16"
                  ...
                  {/* 对话中指示 */}
                  {agent.is_active && (
                    <circle cx="22" cy="4" r="4" fill="#4ade80" />
                  )}
                </g>
              ))}
              ...
              {/* 角色名称列表 */}
              {agentsHere.length > 0 && (
                <text x="80" y="88" textAnchor="middle" fill="#888" fontSize="10">
